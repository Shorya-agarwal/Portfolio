export const worksData = [
  {
    ...
    <ul>
      <li><strong>SIMD Acceleration:</strong> Custom AVX2 intrinsics (_mm256_fmadd_ps) process 8 floating-point dimensions per CPU cycle, achieving approximately 6x speedup over scalar execution</li>
      <li><strong>Memory Optimization:</strong> Flat, contiguous memory buffers maximize L1/L2 cache hit rates during linear scans</li>
      <li><strong>HNSW Implementation:</strong> Multi-layered skip-list structure built from first principles for sub-linear search complexity on massive datasets</li>
      <li><strong>Parallelism:</strong> OpenMP integration for throughput-oriented batch processing</li>
    </ul>
    
    <h3>Performance Benchmarks</h3>
    <p><em>10,000 vectors at 1024 dimensions:</em></p>
    <ul>
      <li>Scalar Baseline: 2108ms</li>
      <li>SIMD (AVX2): 380ms <strong>(5.5x faster)</strong></li>
      <li>SIMD + OpenMP: 547ms (thread overhead visible at small N)</li>
      <li>HNSW Graph: 669ms (cache misses from pointer chasing)</li>
    </ul>
    
    <h3>Key Engineering Insight</h3>
    <p>For datasets under 100k vectors, <strong>cache locality dominates</strong>. The AVX2 linear scan outperforms HNSW because it fully saturates memory bandwidth without the random access penalties of graph traversal. This demonstrates that algorithmic complexity isn't everything - hardware characteristics matter immensely.</p>
    
    <h3>Real-World Applications</h3>
    <ul>
      <li>Recommendation systems (finding similar products/users)</li>
      <li>Image and document search (semantic similarity)</li>
      <li>ML model serving (nearest neighbor classification)</li>
      <li>Vector databases (alternatives to Pinecone, Weaviate)</li>
    </ul>`,
